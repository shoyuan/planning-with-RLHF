...
class LBCCollector(AutonomousAgent):
    ...
    def setup(self, path_to_conf_file):
        ...
        if self.log_wandb:
            wandb.init(project='carla_data_phase1_lbc')
    ...
    def run_step(self, input_data, timestamp):
        ...
        with torch.no_grad():
            ...
            pred_locs = self.converter.cam_to_world(pred_locs)
            pred_locs = torch.flip(pred_locs, [-1])
        
        # 获取控制信号
        cmd_value = cmd.value-1 if cmd.value > 0 else 3
        control = self.get_control(_numpy(pred_locs[cmd_value]), cmd_value, spd)
        
        # 保存数据
        if self.num_frames % (self.num_repeat+1) == 0:
            self.rgbs.append(rgb)
            self.pred_locs.append(_numpy(pred_locs[cmd_value]))
            self.world_locs.append(_numpy(pred_locs))
